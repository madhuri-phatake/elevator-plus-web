var blog = {
    ...
    $("#create_blog").on("click", function (event) {
      $('form[id="blog_form"]').validate({
        ...
        messages: {
          ...
          // },
          blog_meta_tags:{
            required: "<span style='font-size:10px; color: red;'>Please blog meta tags</span>",
          },
          priority: {
            required:
  ...
  add_blog: function () {
    ...
    let ur = $("#blog_url").val();
    if(ur.length >= 70){
      toastr["error"]("Max characters 70 ", "error");
      return false;
    }else{
      ...
      for (let i = 0; i < event.target.files.length; i++) {
        ...
        img.onload = async () => {
          ...
          if (imageFile.size <= 1000000) {
            let filePath = "vendor/" + data["subfolder-name"] + "/" + date + "/" + uuidv4() + "." +Extension;
            await s3.upload(
                {
                  Key: filePath,
                  Body: imageFile,
...
async function UPLOAD_THUMBNAIL_ICON(dataArray, event) {
  ...
  let fuData = document.getElementById(data["id"]);
      ...
      for (let i = 0; i < event.target.files.length; i++) {
        ...
        img.onload = async () => {
          ...
          if (imageFile.size <= 10000000) {
            let filePath =  data["subfolder-name"] + "/" + date + "/" + uuidv4() + "." +Extension;
            await s3.upload(
                {
                  Key: filePath,
                  Body: imageFile,
